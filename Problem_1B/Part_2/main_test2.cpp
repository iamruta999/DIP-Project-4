...
//Problem 1B_Part2
// This code tests the optical character recognition (OCR) system for the test image 2
#include <iostream>
#include <stdio.h>
#include <stdlib.h>
#include <cmath>
...
int main()
{
    ...
    int height = 400;
    int bytesPerPixel = 3;
    char *argv[] = {"Test_ideal2.raw","preProcessedImage.raw","segment1.raw","segment2.raw","segment3.raw","segment4.raw","segment5.raw","segment6.raw","segment7.raw","segment8.raw","segment9.raw","segment10.raw","segment11.raw","segment12.raw"};
    ...
    threshold(gray, thresholdedImage1, 50, 255, THRESH_BINARY);
    imshow("image",thresholdedImage1);
    ...
    cvtColor(thresholdedImage, thresholdedColor, COLOR_GRAY2BGR);
    rectangle(thresholdedColor, boundary, Scalar(255,0,0),2,8,0);
    // Remove the boundary
    // Upper part
    for (int row = boundary.y - 10; row < boundary.y + 25; row++)//row = 0; row < boundary.y + 25; row++)
    {
    ...
    // lower part
    for (int row = boundary.y + boundary.height + 10; row > boundary.y + boundary.height - 25; row--)
    {
        for (int column = boundary.x; column < boundary.x + boundary.width; column++ )
        {
            thresholdedImage.at<int>(row,column) = 0;
    ...
    for (int row = boundary.y; row < boundary.y + boundary.height; row++)
    {
        for (int column = width-1; column > width - 6; column-- )
        {
            thresholdedImage.at<int>(row,column) = 0;
        }
    }
